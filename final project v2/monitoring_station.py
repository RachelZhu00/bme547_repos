...
def design_window():
    ...
    def find_patient():
        ...
        if answer["ecg_images"] != '':
            latest_ecg_str = answer["ecg_images"][-1]
            filename = "Patient " + mrn + " ECG Image " + answer["ecg_time"][-1]
            save_b64_image(latest_ecg_str, filename)
            filepath = "image/" + filename + ".jpg"
            im = load_and_resize_image(filepath)
            ecg_image_label1.configure(image=im)
            ecg_image_label1.image = im

        if answer["medical_images"] != '':
            latest_mi_str = answer["medical_images"][-1]
            filename = "Patient " + mrn + " Medical Image " + answer["mi_time"][-1]
            save_b64_image(latest_mi_str, filename)
            filepath = "image/" + filename + ".jpg"
            im = load_and_resize_image(filepath)
            m_image_label1.configure(image=im)
    ...
    ecg_image1 = load_and_resize_image("image/blank_pic.jpeg")
    ...
    ecg_image2 = load_and_resize_image("image/blank_pic.jpeg")
    ...
    ttk.Label(root, text="Latest Medical Images").grid(column=0, row=15)
    m_image1 = load_and_resize_image("image/blank_pic.jpeg")
    m_image_label1 = ttk.Label(root, image=m_image1)
    m_image_label1.grid(column=0, row=16)

    ttk.Label(root, text="Selected Medical Images").grid(column=1, row=15)
    m_image2 = load_and_resize_image("image/blank_pic.jpeg")
    ...
    def ecg_display():
        ...
        filename = "Patient " + mrn + " ECG Image " + time
        save_b64_image(image_str, filename)
        filepath = "image/" + filename + ".jpg"
        im = load_and_resize_image(filepath)
        ecg_image_label2.configure(image=im)
        ecg_image_label2.image = im
    ...
    def m_display():
        ...
        filepath = "image/" + filename + ".jpg"
        im = load_and_resize_image(filepath)
        m_image_label2.configure(image=im)
